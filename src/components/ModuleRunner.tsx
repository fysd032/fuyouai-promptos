...
setActiveAttachmentId(att.id);
    ...
    if (file.size > MAX_BYTES) {
      alert(`File too large: ${formatBytes(file.size)}, current limit is ${formatBytes(MAX_BYTES)}`);
      return;
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-blue-400">
              Variant: {moduleData.variant.label ?? "Default"}
            </div>
      ...
      <div className="p-6 flex flex-col gap-4 flex-1 min-h-0">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="text-xs text-gray-400">
            ...
            {attachments.length ? <span className="ml-2 text-gray-500">(will be included when running)</span> : null}
          </div>
        ...
        >
          <Play size={16} /> {status === "loading" ? "Running..." : "Run"}
        </button>
