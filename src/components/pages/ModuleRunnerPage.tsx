...
import { StatusFeedback, FeedbackStatus } from "@/src/components/StatusFeedback";
import { MODULES_DB } from "@/src/data/universalModules";
import { INDUSTRY_TEMPLATES_DB } from "@/src/data/industryTemplates";
import { callPromptOS } from "@/src/lib/promptos";
...
const ModuleRunnerPage: React.FC = () => {
  ...
  // Look up module info in both universal modules and industry templates
  const moduleInfo =
  ...
  const handleRun = async () => {
    try {
      ...
      /**
       * ModuleRunner usage:
       * - moduleKey is the promptKey (e.g. A1-01 / industry-xxx / general-xxx)
       * - Calls callPromptOS (which internally uses NEXT_PUBLIC_API_BASE or apiBase param)
       */
      ...
      // Handle different response structures (backend/cloud may use different field names)
      ...
      if (!ok) {
        const err =
          ...
          resp?.message ??
          resp?.raw?.error ??
          "API call failed (returned ok=false)";
    ...
    <div className="flex flex-col h-full max-w-6xl mx-auto pb-10">
      ...
      <div className="flex items-center justify-between mb-6 pt-2">
        <div className="flex items-center gap-2 text-sm text-[#9CA3AF]">
          ...
          <span className="text-[#F9FAFB]">Run Console</span>
        </div>
        <Link
          ...
          <div className="bg-[#111827] border border-[#1F2937] rounded-2xl flex-1 flex flex-col shadow-lg relative overflow-hidden h-full">
            <div className="px-5 py-4 border-b border-[#1F2937] flex items-center justify-between bg-[#1F2937]/30">
              <div className="flex items-center gap-2 text-sm font-medium text-[#F9FAFB]">
                <Sparkles size={16} className="text-emerald-400" /> AI Output
              </div>
              {result && (
                <button
                  onClick={handleCopy}
            ...
            <div className="flex-1 p-0 overflow-y-auto custom-scrollbar relative bg-[#0A0F1C]/50">
              {result ? (
                <div className="p-6">
                  {/* Optional: debug view of final prompt */}
                  {generatedPrompt && (
                    <details className="mb-6 group">
                      <summary className="text-[10px] uppercase tracking-widest text-[#6B7280] cursor-pointer hover:text-[#3B82F6] list-none flex items-center gap-2 mb-2">
                        <Terminal size={10} /> View Actual Prompt Used
                      </summary>
